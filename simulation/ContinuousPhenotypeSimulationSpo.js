...
const comXField = new Float64Array(width * height); // Initialize comXField with the grid size

for (let i = 0; i < width; i++) {
    for (let j = 0; j < height; j++) {
        const index = i + j * width;
        comXField[index] =(height- j)* 0.01; // Example initialization, adjust as needed
    }
}
...
export const updateBacteriaCytoplasmSpo = (currentBacteria, concentrationsState, cytoplasmManager, HEIGHT, WIDTH, timeLapse) => {
    // Ensure comXField is initialized only once
   concentrationsState.concentrationField = comXField; // Assign comXField to concentrationsState
        
    //const concentrations = concentrationsState.concentrationField;
    const concentrations = comXField; // Using comXField for concentrations
